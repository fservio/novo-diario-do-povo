...
import type { InstagramPublication, InstagramStoryVariant } from './types'

function imageUrl(baseUrl: string, key: string | null, focalX: number, focalY: number, width: number, height: number): string {
  if (!key) return ''
  if (/^https?:\/\//i.test(key)) return key
  return `${baseUrl.replace(/\/$/, '')}/i/${key.replace(/^\//, '')}?w=${width}&h=${height}&q=92&fp-x=${focalX / 100}&fp-y=${focalY / 100}`
}
...
export function renderInstagramArtwork(publication: InstagramPublication, baseUrl: string): string {
  const cover = imageUrl(baseUrl, publication.cover_media_url, publication.image_position_x, publication.image_position_y, 1080, 1350)
...
<head>
  ...
  <style>
    *{box-sizing:border-box}html,body{width:1080px;height:1350px;margin:0;overflow:hidden;background:#0b2539}
    ...
    .photo{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:${publication.image_position_x}% ${publication.image_position_y}%;transform:scale(1.12);transform-origin:${publication.image_position_x}% ${publication.image_position_y}%}
...
function storyTitleSize(title: string): number {
...
export function renderInstagramStoryArtwork(
  publication: InstagramPublication,
  story: InstagramStoryVariant,
  baseUrl: string
): string {
  const cover = imageUrl(baseUrl, publication.cover_media_url, story.image_position_x, story.image_position_y, 1080, 1920)
  const hat = story.hat || publication.category_name || 'Notícia'
  const subtitle = story.subtitle || ''
  const credit = story.photo_credit || publication.photo_credit || ''
  const fontSize = storyTitleSize(story.title)
...
<head>
  ...
  <meta name="viewport" content="width=1080,initial-scale=1">
  <meta name="robots" content="noindex,nofollow,noarchive">
  <title>Story editorial | Diário do Povo</title>
  <style>
    *{box-sizing:border-box}html,body{width:1080px;height:1920px;margin:0;overflow:hidden;background:#0b2539}
    body{font-family:Arial,Helvetica,sans-serif;-webkit-font-smoothing:antialiased}
    .art{position:relative;width:1080px;height:1920px;overflow:hidden;background:#173f5f;color:#fff}
    .photo{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:${story.image_position_x}% ${story.image_position_y}%;transform:scale(1.12);transform-origin:${story.image_position_x}% ${story.image_position_y}%}
    .fallback{position:absolute;inset:0;background:radial-gradient(circle at 80% 10%,#315d7c 0,transparent 42%),linear-gradient(145deg,#183f5d,#071d2d)}
    .wash{position:absolute;inset:0;background:linear-gradient(180deg,rgba(3,18,29,.05) 0%,rgba(3,18,29,.1) 28%,rgba(4,20,31,.72) 58%,rgba(4,20,31,.98) 82%,rgba(4,20,31,.92) 100%)}
    .top{position:absolute;top:296px;left:0;right:0;display:flex;align-items:center;justify-content:space-between;padding:0 64px}
    .brand img{display:block;width:184px;height:auto;filter:brightness(0) invert(1);opacity:.97}
    .edition{font-size:13px;font-weight:700;letter-spacing:2.8px;text-transform:uppercase}
    .copy{position:absolute;left:64px;right:64px;bottom:410px}
    .rule{width:82px;height:7px;margin-bottom:26px;background:#d1a85d}
    .hat{margin:0 0 20px;color:#e2bd78;font-size:22px;font-weight:800;letter-spacing:3.8px;text-transform:uppercase}
    h1{max-width:950px;margin:0;font-family:Georgia,'Times New Roman',serif;font-size:${fontSize}px;font-weight:700;letter-spacing:-3px;line-height:1.01;text-wrap:balance;text-shadow:0 2px 20px rgba(0,0,0,.28)}
    .subtitle{max-width:900px;margin:27px 0 0;color:#e8edf0;font-size:30px;font-weight:500;line-height:1.3;text-wrap:balance}
    .cta{margin:30px 0 0;color:#e2bd78;font-size:17px;font-weight:800;letter-spacing:1.7px;text-transform:uppercase}
    .footer{display:flex;align-items:flex-end;justify-content:space-between;gap:30px;margin-top:28px;padding-top:22px;border-top:1px solid rgba(255,255,255,.28)}
    .site{font-size:15px;font-weight:800;letter-spacing:1.4px}.credit{max-width:430px;color:#d1d9de;font-size:12px;letter-spacing:.4px;text-align:right}
  </style>
</head>
<body>
  <main class="art" aria-label="Story para Instagram do Diário do Povo">
    ${cover ? `<img class="photo" src="${escapeHtml(cover)}" alt="">` : '<div class="fallback"></div>'}
    <div class="wash"></div>
    <header class="top"><div class="brand"><img src="/static/logo-dp.png" alt="Diário do Povo"></div><div class="edition">Jornalismo independente</div></header>
    <section class="copy">
      ...
      ${subtitle ? `<p class="subtitle">${escapeHtml(subtitle)}</p>` : ''}
      <p class="cta">${escapeHtml(story.cta_text || 'Leia a matéria completa')}</p>
      <footer class="footer"><span class="site">JORNALDIARIODOPOVO.COM.BR</span><span class="credit">${credit ? `Foto: ${escapeHtml(credit)}` : 'Crédito não informado'}</span></footer>
    </section>
  </main>
</body>
