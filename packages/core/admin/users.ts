...
function renderUsersList(
  ...
  const rows = users.map(user => `
    <tr>
      ...
      <td data-label="Autoria">${user.author_id ? `<a class="management-author-link" href="/admin/authors/${user.author_id}">${escapeHtml(user.author_name || 'Perfil editorial')}</a>` : `<form method="POST" action="/admin/users/${user.id}/ensure-author"><input type="hidden" name="csrf" value="${csrfToken}"><button class="management-link-button" type="submit">Criar perfil</button></form>`}</td>
    ...
    <section class="management-page management-page--narrow">
      ...
      <div class="management-form-grid">
        <form class="card management-form" method="POST" action="${isEdit ? `/admin/users/${user!.id}` : '/admin/users'}">
          <input type="hidden" name="csrf" value="${csrfToken}"><div class="management-card-title"><span>Identidade e função</span><small>Campos obrigatórios</small></div>
